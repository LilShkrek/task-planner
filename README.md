...
- `ml-service/` - Python REST service для анализа задачи. Внутри используется учебная PyTorch-модель: входные признаки задачи, GRU, dense/perceptron-блок, выходные scores по методам и параметры планирования. Методы тайм-менеджмента и шаблоны планов загружаются из PostgreSQL, затем шаги управляемо адаптируются под тип задачи.
...
### Адаптация шагов плана

Генератор не пишет свободный текст с нуля. Он:

- берет базовые шаги из JSON-шаблона в PostgreSQL;
- определяет тип задачи по ключевым словам в `title`, `description`, `context`;
- если тип распознан, заменяет заголовки и описания шагов на более предметные заготовки;
- если тип не распознан, оставляет базовый шаблон без изменений.

Сейчас поддерживаются типы:

- презентация, слайды, выступление;
- письмо, почта, ответ;
- учебная работа, глава, конспект;
- звонок, банк, документы;
- набор мелких дел.
