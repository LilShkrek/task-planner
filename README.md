...
Текущая версия использует multi-method planning: GRU/dense-модель считает `scores` для всех методов, а отдельный selection-модуль выбирает 3-5 методов с разными группами и ролями. Старое поле `method_code` остается в ответе как основной метод для совместимости, но рядом возвращаются `ranked_methods`, `selected_methods` и `explanation`.

Комбинированный план строится этапами: один метод может отвечать за уточнение цели, другой за приоритизацию, третий за декомпозицию, четвертый за распределение времени или выполнение, пятый за контроль результата.
