...
Текущая версия использует multi-method planning: GRU/dense-модель считает `scores` для всех методов, а отдельный selection-модуль выбирает 3-5 методов с разными группами и ролями. При выборе учитывается semantic compatibility метода с задачей: домен, подцели, оценка длительности, срочность и сложность. Основной результат выбора - `selected_methods`. Поле `ranked_methods` остается диагностическим, а старые `method_code`, `method_name`, `confidence` сохранены только для совместимости.
...
Тексты шагов дополнительно выравниваются с назначенным методом. Для крупных задач вроде отпуска, исследования или презентации методы старта не вытесняют более естественные методы распределения времени только из-за высокого score. Подцели нормализуются для пользовательского текста: `даты` -> `выбор дат поездки`, `жилье` -> `выбор жилья`, `вещи и документы` -> `подготовка вещей и проверка документов`.
