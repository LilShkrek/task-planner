...
- `ml-service/` - Python REST service для анализа задачи. Внутри используются модель выбора метода `GRU + dense/perceptron` и локальная предобученная text-to-text модель для генерации русского текста ответа. Каталог из 40 методов тайм-менеджмента и шаблоны планов загружаются из PostgreSQL.
...
Текущая версия по-прежнему использует базовый single-method selection: модель выбирает один `method_code`. Каталог подготовлен для следующего этапа - multi-method planning, где один план сможет комбинировать несколько методов.
...
3. Реализовать multi-method planning на основе расширенного каталога методов.
4. Расширить учебный датасет и добавить метрики качества модели.
5. Добавить frontend.
